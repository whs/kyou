widgets["gallery"] = Widget.extend({
	...
	renderer: Backbone.View.extend({
		...
		render: function(opt){
			...
			}else if(this.model.get("display") == "touch"){
				this.el.innerHTML = "<div class='flexslider'><ul class='slides'></ul></div>";
				_.each(this.model.get("images"), function(v){
					var el = $("<li><img></li>");
					el.find("img").attr("src", !opt ? "/bookfiles/" + page.project.id + "/" + v.img : v.img);
		...
		javascripts: function(opt){
			var files = {
				...
				"images": [],
				"touch": ["assets/jquery.js", "files/jquery.flexslider-min.js"],
			}
			var out = files[this.model.get("display")] || [];
			if(this.model.get("display") == "slider"){
				var config = {
				...
				var config = {
					"animation": this.model.get("touchanim"),
					"direction": this.model.get("touchdir"),
					"animationLoop": !!this.model.get("loop"),
					//"smoothHeight": true,
					"animationSpeed": parseInt(this.model.get("animSpeed")),
					"slideshow": this.model.get("pauseTime") <= 0 ? false: true,
					"slideshowSpeed": parseInt(this.model.get("pauseTime")),
					"directionNav": !!this.model.get("controls"),
				};
				config = JSON.stringify(config);
				out.push("$(function(){\n$('#"+this.model.get("id")+" .flexslider').flexslider("+config+");\n});");
			}
			return out;
		},
		resources: function(opt){
			var files = {
				"touch": ["files/images/bg_direction_nav.png"],
			}
			var out = files[this.model.get("display")] || [];
			return out;
		}
