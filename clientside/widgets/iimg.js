	...
	renderer: Backbone.View.extend({
		...
		render: function(opt){
			...
			_.each(this.config.items || [], function(v){
				...
				var effectdata = "";
				if(v.effect && v.effect != "null"){
					effectdata = {
						effect: v.effect,
						duration: parseFloat(v.fadeduration)
					};
					if(v.effect == "slide" || v.effect == "slidefade"){
						effectdata.initleft = parseFloat(v.initx);
						effectdata.inittop = parseFloat(v.inity);
						effectdata.top = v.txttop;
						effectdata.left = v.txtleft;
					}
					effectdata = JSON.stringify(effectdata).replace(/"/g, "'"); // hack
				}
				if(v.action == "bg" && v.bgimg){
					...
					el.addClass("hastxthover").attr("data-effect", effectdata);
				}else if(v.action == "img" && v.image){
					$("<img>").addClass("imghover").attr("src", v.image).css({
						top: v.txttop || 0,
						left: v.txtleft || 0,
					}).insertAfter(el);
					el.addClass("hasimghover").attr("data-effect", effectdata);
				}
				if(v.icon){
